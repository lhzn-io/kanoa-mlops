...
`kanoa-mlops` is the infrastructure companion to the `kanoa` library. It provides the "brawn" (compute, storage, serving) for local AI interpretation.
...
### Scripts (Bash/Python)
- **Idempotency**: Scripts should be safe to run multiple times (check if dir exists before creating).
- **Error Handling**: Use `set -e` in bash scripts.
- **Environment Variables**: Use `.env` files for secrets; providing `env.example` is mandatory.

### Documentation
- **Prerequisites**: Clearly state hardware requirements (e.g., "Requires NVIDIA GPU with 24GB VRAM").
