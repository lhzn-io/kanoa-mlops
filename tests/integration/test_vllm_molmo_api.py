...
def test_boardwalk_photo():
    ...
    try:
        ...
        description, usage, duration_s = query_molmo("Describe this image in detail.", base64_url)
        ...
        print(f"[OK] Response:\n{description}")
        print(f"[PERF] Duration: {duration_s:.2f}s | Tokens: {tokens_generated} | Speed: {tokens_per_sec:.1f} tok/s")
...
def test_data_interpretation():
    ...
    try:
        ...
        assert "increase" in result.lower() or "trend" in result.lower() or "june" in result.lower() or "jun" in result.lower(), \
...
def print_performance_report():
    ...
    print("\nPer-Test Breakdown:")
    print(f"{'Test Name':<25} {'Duration (s)':<15} {'Tokens':<10} {'Speed (tok/s)':<15}")
    print("-"*70)

    for metric in TEST_METRICS:
        print(f"{metric.test_name:<25} {metric.duration_s:>10.2f}      {metric.tokens_generated:>6}     {metric.tokens_per_second:>10.1f}")
...
def save_benchmark_results():
    ...
    results = {
        ...
        "summary": {
            ...
            "avg_tokens_per_second": sum(m.tokens_generated for m in TEST_METRICS) / sum(m.duration_s for m in TEST_METRICS)
        },
